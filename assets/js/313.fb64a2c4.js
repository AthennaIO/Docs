...
;// CONCATENATED MODULE: ./node_modules/mermaid/dist/erDiagram-9d236eb7.js
...
const addEntity = function(name, alias = void 0) {
  if (entities[name] === void 0) {
    ...
    mermaid_8af3addd.l.info("Added new entity :", name);
  } else if (entities[name] && !entities[name].alias && alias) {
    entities[name].alias = alias;
    mermaid_8af3addd.l.info(`Add alias '${alias}' to entity '${name}'`);
  }
...
const addAttributes = function(entityName, attribs) {
  ...
  for (i = attribs.length - 1; i >= 0; i--) {
    ...
    mermaid_8af3addd.l.debug("Added attribute ", attribs[i].attributeName);
  }
};
const addRelationship = function(entA, rolA, entB, rSpec) {
  ...
  mermaid_8af3addd.l.debug("Added new relationship :", rel);
};
const getRelationships = () => relationships;
const clear = function() {
  entities = {};
  relationships = [];
  (0,mermaid_8af3addd.t)();
};
const erDb = {
  Cardinality,
  Identification,
  getConfig: () => (0,mermaid_8af3addd.c)().er,
  addEntity,
  addAttributes,
  getEntities,
  addRelationship,
  getRelationships,
  clear,
  setAccTitle: mermaid_8af3addd.s,
  getAccTitle: mermaid_8af3addd.g,
  setAccDescription: mermaid_8af3addd.b,
  getAccDescription: mermaid_8af3addd.a,
  setDiagramTitle: mermaid_8af3addd.q,
  getDiagramTitle: mermaid_8af3addd.r
};
const ERMarkers = {
...
const drawAttributes = (groupNode, entityTextNode, attributes) => {
  ...
  attributes.forEach((item) => {
    ...
    let nodeHeight = 0;
    const attributeType = (0,mermaid_8af3addd.v)(item.attributeType);
    const typeNode = groupNode.append("text").classed("er entityLabel", true).attr("id", `${attrPrefix}-type`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", (0,mermaid_8af3addd.c)().fontFamily).style("font-size", attrFontSize + "px").text(attributeType);
    const nameNode = groupNode.append("text").classed("er entityLabel", true).attr("id", `${attrPrefix}-name`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", (0,mermaid_8af3addd.c)().fontFamily).style("font-size", attrFontSize + "px").text(item.attributeName);
    const attributeNode = {};
    ...
    if (hasKeyType) {
      ...
      const keyTypeNode = groupNode.append("text").classed("er entityLabel", true).attr("id", `${attrPrefix}-key`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", (0,mermaid_8af3addd.c)().fontFamily).style("font-size", attrFontSize + "px").text(keyTypeNodeText);
    ...
    if (hasComment) {
      const commentNode = groupNode.append("text").classed("er entityLabel", true).attr("id", `${attrPrefix}-comment`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", (0,mermaid_8af3addd.c)().fontFamily).style("font-size", attrFontSize + "px").text(item.attributeComment || "");
...
const drawEntities = function(svgNode, entities2, graph) {
  ...
  keys.forEach(function(entityName) {
    ...
    const textNode = groupNode.append("text").classed("er entityLabel", true).attr("id", textId).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "middle").style("font-family", (0,mermaid_8af3addd.c)().fontFamily).style("font-size", conf.fontSize + "px").text(entities2[entityName].alias ?? entityName);
...
const drawRelationshipFromLayout = function(svg, rel, g, insert, diagObj) {
  ...
  const labelNode = svg.append("text").classed("er relationshipLabel", true).attr("id", labelId).attr("x", labelPoint.x).attr("y", labelPoint.y).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-family", (0,mermaid_8af3addd.c)().fontFamily).style("font-size", conf.fontSize + "px").text(rel.roleA);
...
const draw = function(text, id, _version, diagObj) {
  conf = (0,mermaid_8af3addd.c)().er;
  mermaid_8af3addd.l.info("Drawing ER diagram");
  const securityLevel = (0,mermaid_8af3addd.c)().securityLevel;
  let sandboxElement;
  ...
  mermaid_8af3addd.u.insertTitle(svg, "entityTitleText", conf.titleTopMargin, diagObj.db.getDiagramTitle());
  const svgBounds = svg.node().getBBox();
  const width = svgBounds.width + padding * 2;
  const height = svgBounds.height + padding * 2;
  (0,mermaid_8af3addd.i)(svg, height, width, conf.useMaxWidth);
