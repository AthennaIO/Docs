...
Let's add some configuration to the `@Service()` annotation of
...
When bootstrapping the Athenna application the `StringNormalizer` class
will be registered in the service container as a
[singleton](/docs/architecture-concepts/service-container#binding-a-singleton)
with the alias `App/Services/StringNormalizer` and the camel alias
`stringNormalizer`, that is typically used in
[automatic constructor injections](/docs/architecture-concepts/service-container#automatic-constructor-injection)
and also by [the `@Inject()` annotation](/docs/architecture-concepts/service-container#using-inject-annotation) when no alias is provided to it.
