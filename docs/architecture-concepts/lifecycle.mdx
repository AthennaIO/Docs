...
The entry point for all types of an Athenna application is the `bootstrap/main.js` file. The first action taken by Athenna
...
service providers for the application are configured in the `config/app.js` configuration file's providers array.
...
be run before the `routes/console.js` file is preloaded. These bootstraps configure error handling, configure logging, register custom
...
Typically, these classes handle internal Athenna configuration that you do not need to worry about.

The Kernel of `Console` folder also defines a list of [`commands`](/docs/the-basics/cli/commands) that will be registered
before the application bootstrap and before preloading `routes/console.js`. See the example:
...
In Athenna you can use the loaders helpers such as `TestCommandsLoader` to load all commands and custom templates from
each package.

But as you can see we have a little logic here to separate what is an internal command of Artisan of what is a command of
...
the internal Artisan commands like `node artisan make:test` would be there.

The Kernel of `Console` folder also defines a list of [`templates`](/docs/the-basics/cli/commands#id-registering-custom-templates) that you can set to use inside your commands using the
`TemplateHelper` class from `@athenna/artisan`. See the example:
