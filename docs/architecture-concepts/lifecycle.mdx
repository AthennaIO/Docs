...
be run before the `routes/console` file is preloaded. These bootstraps configure error handling, configure logging, register custom
...
In Athenna you can use the loaders helpers such as `HttpCommandsLoader` to load all commands and custom templates from
each package.
