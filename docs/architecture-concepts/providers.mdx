...
The Athenna CLI can generate a new provider via the `make:provider` command:

```shell
athenna make:provider MyAppProvider
```
...
export class MyAppProvider extends ServiceProvider {
   ...
   */
  public boot(): void {
    const myAppRepository = this.container.safeUse<MyAppRepository>('MyApp/Repository')
