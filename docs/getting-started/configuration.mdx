...
the `Env` function has the third parameter called `autoCast` that by default is always `true`, if you set as `false` it
will not cast the value for you.
...
console.log(Env<boolean>('APP_DEBUG', defaultValue)) // true
console.log(Env<string>('APP_DEBUG', defaultValue, autoCast)) // '(true)'
```
...
```typescript
console.log(Env<string>('APP_DOMAIN')) // "http://localhost:1335"
```
...
```typescript
console.log(Config.get<string>('app.name', 'Athenna')) // MyAppName
```
