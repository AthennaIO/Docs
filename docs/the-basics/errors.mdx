...
The `debug` option in your `config/app` configuration file determines how much
...
By default, Athenna uses the `exception` channel of your `config/logging` file to log all
exceptions that happens in your application.

:::tip

You can change the driver and formatter of `exception` channel inside `config/logging`
...
to the `handle` method of `app/Http/Exceptions/Handler` class. By default,
...
using the `ignoreCodes` and `ignoreStatuses` properties of your `app/Http/Exceptions/Handler`
class.
...
be considered as an not treated exception.
