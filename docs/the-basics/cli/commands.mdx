...
export class SendEmails extends Command {
   ...
   */
  protected description = 'Display welcome object.'
  ...
  public constructor(container: Container) {
    ...
    this.mailgunService = container.mailgunService
  }
...
export class SendEmails extends Command {
   ...
   * @return {Promise<void>}
   */
  async handle(email: string, options: any): Promise<void> {
    const message = 'Hello from Athenna!'
...
"console kernel". The `CommandsProvider` will scan the `app/Console/Commands` directory and automatically register each
command it contains with Artisan. By default, Athenna will come with some default Artisan commands registered inside `commands` property:
