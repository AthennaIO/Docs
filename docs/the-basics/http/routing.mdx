...
however, you can instruct Athenna to display the route middleware by adding the `-v` option to the command:

```shell
athenna route:list -v
```

In addition, you may instruct Athenna to hide any routes that are defined by third-party packages by providing the
`--except-packages` option when executing the `route:list` command:

```shell
athenna route:list --except-packages
```
...
```typescript
Route.get('/posts/{post}/comments/{comment}', ({ response, params }) => {
...
To assign [`middlewares`](/docs/the-basics/http/middleware) to all routes within a group, you may use the `middleware`
...
the configurations that `fastify-rate-limit` supports can be set inside `config/http` file in the `rateLimit` object.

Rate limit plugin is registered in your http application by default, but you can remove it setting the `noRateLimit` as
`true` in `config/http` file.

:::tip
For more information on Rate limit and Rate limit headers, please consult the
...
that `fastify-cors` supports can be set inside `config/http` file in the `cors` object.
