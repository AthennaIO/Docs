...
This command will place a new `EnsureApiKeyIsValid` class within your `app/Http/Middlewares` directory. In this middleware,
...
export class EnsureApiKeyIsValid {
  ...
  /**
    ...
    * @return {any}
    */
  handle({ queries, next }: HandleContextContract) {
...
property of your `app/Http/Kernel.js` class.
...
`app/Http/Kernel.js` file. You can do that inside `namedMiddlewares` property, you may add your own middleware to this list
...
The requests log created by Athenna when the `logRequests` property is true in your `config/http.js` file are handled by a
terminable middleware! You can see the code [here](https://github.com/AthennaIO/Http/blob/develop/src/Kernels/HttpKernel.js).
