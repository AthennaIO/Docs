...
When an incoming request matches the specified route URL, the `show` method on the `app/Http/Controllers/ UserController`
...
Because of this common use case, Athenna resource routing assigns the typical create, read, update, and delete ("CRUD")
...
```shell
athenna make:controller PhotoController --resource
```
