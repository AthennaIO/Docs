...
If the value is present in your object, but its values is `null`, it will be set in your object resource. Only the
...
:::warning
If you don't load your relationships before calling your resource class, it will be set as `null` in the json when
using the `toJson` method and set as an empty array when using the `toArray` method:
