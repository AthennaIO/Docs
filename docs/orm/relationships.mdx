...
As you can see you can use the `includes` method to obtain an instance of the relationship without actually executing a query to load the related models.
In addition, when you include a model you can make more queries using the relation model inside the father model.
...
const user = await User.query().includes('posts').find()
const posts = user.posts

const promises = posts.map(post => post.delete())
```

### Sub-queries in relationships {#id-sub-queries-in-relationships}

Also, the `includes` method can receive a closure as second parameter where you can define a lot of queries for when including your relationships. In the
...
const user = await User.query()
    .includes('posts', query => query.includes('comments').where('active', 1))
    .findMany()
...
:::tip
The `query` value in `includes` closure will be an instance of `ModelQueryBuilder` but of the relation. So you can use all its methods too.
:::
