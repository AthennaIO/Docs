...
- #### [Querying relationships](#id-querying-relationships)
    ...
    - #### [Querying relationships existence](#id-querying-relationships-existence)
...
blog posts that have at least one comment. To do so, you may pass the name of the relationship to the `has` method:
...
Nested `has` statements may be constructed using "dot" notation. For example, you may retrieve all posts that have at least one comment that has at
least one image:
...
If you need even more power, you may use the `whereHas` method to define additional query constraints on your `has` queries, such as inspecting the
...
### Manipulating relationships with instance methods {#id-manipulating-relationships-with-instance-methods}

Instead of using relationships queries methods you can modify the relation directly in the parent model instance. In the example bellow the `User` model
belongs to an `Account`, let's see how we can update the username using an instance of the `Account` model:
...
:::tip
As you can see in the example above we are calling the `save` method in the `user` relationship of `account`. Always remember that all the instance
methods of the models like `save`, `load`, `refresh`, etc... will be available in relationships instances too.
:::
...
#### Using instance properties in many-to-many {#id-using-instance-properties-in-many-to-many}
