...
- #### [Defining relationships queries in model](#id-defining-relationships-queries-in-model)
    - #### [Finding one or multiples relationships](#id-finding-one-or-multiples-relationships)
    - #### [Creating a new record in the relationship](#id-creating-a-new-record-in-the-relationships)
    - #### [Updating records in the relationship](#id-updating-records-in-the-relationships)
    - #### [Deleting records in the relationship](#id-deleting-records-in-the-relationships)
...
Once the relationship is defined, we may retrieve the related record using the `with` method of the model query builder:
...
Once the relationship property has been defined, we can access the related `comments` by including it with the `with` method and accessing the
`comments` property:
...
Once the relationship is defined, you may access the user's roles using the `with` method:
...
As you have already learned, working with belongs-to-many relations requires the presence of an intermediate table. Athenna provides some very helpful ways
...
When you use the `postsQuery` you are only going to be able to perform queries where the `posts` belongs to the `user`. Let's check some examples of how
to use it starting by how to **find one** and **find many** `posts`:

```javascript
const user = await User.find()

const activePostsOfUser = await user.postsQuery()
    .where('active', 1)
    .findMany()

const latestPostOfUser = await user.postsQuery()
    .where('active', 1)
    .latest()
    .find()
```

If you use the `dump` method to check to query you will se that there already a where clause there that Athenna has put to retrieve only the `posts` that
belongs to the `user`:
...
```shell
select * from posts where userId = 1 and where active = 1;
```

### Creating a new record in the relationship {#id-creating-a-new-record-in-the-relationship}

You can create new records in the relationship using the `create`, `createMany` or `createOrUpdate` methods. Just like the above example, Athenna will
automatically set the values in these methods that will define that a post will be created for that specific user. In other words, in the example bellow
you do not need to define the `userId` when creating a new product using the `postsQuery` method, Athenna will automatically do it for you.

```javascript
const user = await User.find()
const newPost = await user.postsQuery().create({ title: 'New post' })
```

:::warning
The `newPost` will not be available in your `user` constant when using the `postsQuery` method. To "refresh" your user data you can use the `load` method
that will retrieve all the posts of the user again:
...
If you have already loaded before your posts, you can use the `refresh` method:

```javascript
const user = await User.query().with('posts').find()

// This post will not be available in the "user.posts"
const newPost = await user.postsQuery().create({ title: 'New post' })

// Now the "newPost" is available
await user.refresh()
```
:::

### Updating records in the relationship {#id-updating-records-in-the-relationship}

Coming soon...

### Deleting records in the relationship {#id-deleting-records-in-the-relationship}
