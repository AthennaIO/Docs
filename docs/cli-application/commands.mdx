...
export class Greet extends BaseCommand {
  ...
  public async handle() {
    this.logger.simple('Hello', 'World!')

    this.logger.update('This will not', 'be rendered')
    this.logger.update('Because we are updating', 'it here')
...
<img 
  src='/img/examples/artisan-ui.png' 
  alt='Artisan UI' 
  style={{
    width: '400px',
    display: 'flex',
