...
- #### [Joins](#id-joins)
- #### [Basic where clauses](#id-basic-where-clauses)
    ...
    - #### [Logical grouping](#id-logical-grouping)
- #### [Advanced where clauses](#id-advanced-where-clauses)
  - #### [Where exists clauses](#id-where-exists-clauses)
- #### [Ordering, grouping, limit & skip](#id-ordering-grouping-limit-and-skip)
- #### [Conditional clauses](#id-conditional-clauses)
- #### [Pagination](#id-pagination)
- #### [Insert statements](#id-insert-statements)
- #### [Update statements](#id-update-statements)
- #### [Delete statements](#id-delete-statements)
...
### Get the client and query builder of driver {#id-get-the-client-of-driver}
...
use the `raw` method provided by the `Database` facade:
...
Instead of using the `Database.raw` method, you may also use the following methods to insert a raw expression
...
#### `selectRaw` {#id-select-raw}

The `selectRaw` method can be used in place of `select(Database.raw(/* ... */))`. This method accepts an
...
#### `whereRaw / orWhereRaw` {#id-where-raw}
...
#### `havingRaw / orHavingRaw` {#id-having-raw}
...
### Inner join clause {#id-inner-join-clause}
...
### Advanced join clauses {#id-advanced-join-clauses}
...
logical grouping. In fact, you should generally always group calls to the `orWhere` method in parentheses in order
to avoid unexpected query behavior. To accomplish this, you may pass a closure to the `where` method:
...
The `whereExists`, `orWhereExists`, `whereNotExists` and `orWhereNotExists` methods allows you to write "where exists" SQL clauses.
...
#### The `latest` & `oldest` methods {#id-the-latest-and-oldest-methods}
...
#### The `groupBy` & `having` methods {#id-the-groupby-and-having-method}
...
To build more advanced `having` statements, see the [`havingRaw`](/docs/database/query-builder#id-having-raw) method.
...
:::tip
The `offset` method is equivalent to `skip` and the `limit` method is equivalent to `take`. `take` and `skip` are usually
...
role field is present on the incoming request and evaluates to a `truthy` value.
...
You can paginate the results of your database using the `paginate` method. This method support 3 arguments, the first argument
is the page (default value is `0`) it defines the page where your pagination will start, the second is the limit
(default value is `10`) it defines the limit of data that will be retrieved per page and the third one defines the resource url
...
The `meta` param will have information about the pagination such as the total of items finds using that query,
...
The query builder also provides the `create` and `createMany` methods that may be used to insert records into the database
table. The `create` method accepts a record with columns names and values:
...
### Create or update (Upsert)  {#id-create-or-update}

The `createOrUpdate` method will insert records that do not exist and update the records that already exist with new values
that you may specify. The method's first argument consists of the values to insert or update, while the second argument
is the column that uniquely identify records within the associated table (the default is `id`). In the example above we
are going to create a new record in the users table **only if** the `txsoura@athenna.io` email is not already registered
in `users` table:
...
method. The `update` method, like the `create` method, accepts a record with columns names and values indicating the columns
to be updated. You may constrain the update query using where clauses. In the example above we are going to "undo" the soft
delete by searching for all records where the `deletedAt` column is not null and setting it to `null`:
...
### Incrementing & decrementing {#id-incrementing-and-decrementing}

The query builder also provides convenient methods for incrementing or decrementing the value of a given column. Both of
these methods accept at least one argument: the column to modify:
...
The query builder's `delete` method may be used to delete records from the table. You may constrain delete statements by
adding "where" clauses before calling the `delete` method:
...
You may use the `dump` method while building a query to dump the current query bindings and SQL. The `dump` method will
