...
running and reverting** [**migrations**](/docs/database/migrations)**,** [**transactions**](/docs/database/getting-started#id-database-transactions)**,
...
If you don't know what is a transaction you can check [clicking here](/docs/database/getting-started#id-database-transactions)
...
You may use the `startTransaction` method provided by the `Database` facade to run a set of operations within a database
transaction. You can use a `try/catch` block to verify if an exception is thrown within your try block, and roll back if
some error occurs. If the closure executes successfully, the transaction should be committed:
