...
`Path.config('http.js')` file in the `rTracer` object:
...
You can also disable the tracer by setting `http.trace` to `false`:

```typescript filename="Path.config('http.js')"
export default {
  trace: false,
}
```

Or by setting the `tracer` option as `false` when booting the 
