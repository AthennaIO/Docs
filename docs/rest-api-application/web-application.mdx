...
If you want to use SSR we recommend bootstrapping an application choosing
`WEB REACT SSR` application type which will come with everything already
configured for you.

:::

If you bootstrap your project using the `WEB REACT` application type
your application will not be using SSR by default and would require
some configurations:

#### Add SSR properties

In your `vite` object inside <Path father="config" child="http.ts" />,
add the SSR properties:
