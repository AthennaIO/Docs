...
within your `Path.views()` directory. In this view we 
...
to the name of the view file in the `Path.views()` directory as defined
...
option inside of the `Path.config('view.ts')` file:
...
the `Path.views('components/button.edge')` file:
...
in the `components` property of you `Path.config('view.ts')`
...
`Path.resources('templates')` directory. Any change
