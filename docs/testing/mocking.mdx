...
The storage library ships a `fake` driver that keeps files in memory.
Select it as the default disk in your `.env.test` file:

```dotenv title=".env.test"
STORAGE_DISK=fake
```

Register the Storage facade with the `StorageProvider` and clear the
fake disk after each test:

```typescript title="UploadServiceTest.ts"
import { FakeDriver, StorageProvider } from '@athenna/storage'
...
The fake disk behaves like a real one, so most tests only need to
...
When you need to force a result (a signed URL, a failure), stub the
`FakeDriver` static methods with `Mock`:

```typescript
Mock.when(FakeDriver, 'getSignedUrl').resolve({ url: 'https://fake.storage/put/a.png' })
Mock.when(FakeDriver, 'put').reject(new Error('disk is full'))
```
